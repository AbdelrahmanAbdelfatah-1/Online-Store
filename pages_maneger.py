...
class CartManager :
    ...
    def checkout(self):
        ...
        self.save_products()
        products_total = self.total_price
        self.cart = []
        self.total_price = 0

        return f"✅ Checkout successful. TOTAL = {products_total}"
...
class CategoryPage(tk.Toplevel) :
    ...
    def __init__(self, parent, category_name, title, cart_manager, nav_stack) :
        ...
        self.cart_manager.reload_products()
    ...
    def search_item(self) :
        ...
        idx = binary_search(sorted_by_name, target , key="name" )
        if idx != -1 :
...
class CartPage(tk.Toplevel) :
    def __init__(self, parent, cart_manager, nav_stack , user ):
        super().__init__(parent)
        self.cart_manager = cart_manager
        self.nav_stack = nav_stack
        self.user = user
    ...
    def checkout(self):
        ...
        if result.startswith("✅") :
            ...
            parts = result.split("TOTAL =")
            message = parts[0].strip()
            products_total = float(parts[1])

            city = self.user.get("Governorate")
            fees = self.cart_manager.delivery_fees.get(city)

            messagebox.showinfo(
                "Checkout",
                f"{message}\n"
                f"Products Total: ${products_total}\n"
                f"Delivery to {city}: ${fees}\n"
                f"Final Total: ${products_total + fees}"
            )
        else :
    ...
    def back_to_store(self) :
        prev_win = self.nav_stack.pop()
        prev_win.state("zoomed")
...
class HomePage(tk.Tk) :
    ...
    def __init__(self, cart_manager, nav_stack,user):
        super().__init__()
        self.cart_manager = cart_manager
        self.nav_stack = nav_stack
        self.user = user
    ...
    def create_widgets(self):
        ...
        tk.Label(main_frame, text=f"Welcome {self.user['name']}", font=("Arial", 28, "bold"), fg=COLORS['primary'],
    ...
    def open_cart(self):
        cart_page = CartPage(self, self.cart_manager, self.nav_stack,self.user)
...
def pages_manager_fun(user) :
    nav_stack = Stack()
    cart_manager = CartManager()
    home_page = HomePage(cart_manager, nav_stack ,user)
